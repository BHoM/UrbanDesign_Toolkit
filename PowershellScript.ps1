...
$Urban = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "Urban" in all files
    Write-Host "`nReplacing occurrences of 'Urban' with "$Urban" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'Urban', $Urban | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$Urban":`n"
    ...
    # Get all files and directories containing "Urban" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("Urban"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('Urban') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('Urban') -and $pathExists)
        {
            $newName = $filename.Replace('Urban', $Urban)
